  ...
  return (
    <div className="relative flex h-full w-80 shrink-0 flex-col border-l bg-background select-none">
      ...
      <button
        type="button"
        onClick={onClose}
        className="absolute right-4 top-4 z-10 flex size-8 items-center justify-center rounded-full hover:bg-muted text-muted-foreground transition-all cursor-pointer outline-none active:scale-95"
      ...
      <div className="flex-1 overflow-y-auto px-5 py-6 space-y-6">
        ...
        <div className="flex flex-col items-center text-center mt-4">
          ...
          <h3 className="mt-3 text-lg font-bold text-foreground truncate max-w-full px-2">
            {friend.displayName}
          </h3>
          ...
          {isGroup ? (
            <>
              ...
              <button
                ...
                className="flex flex-col items-center gap-1.5 text-muted-foreground hover:text-foreground active:scale-95 transition-all outline-none cursor-pointer"
              >
              ...
              {(activeConversation?.members?.find((m) => m.userId === currentUserId)?.role === "owner" ||
              ...
              <button
                ...
                className="flex flex-col items-center gap-1.5 text-destructive hover:text-destructive/80 active:scale-95 transition-all outline-none cursor-pointer"
              >
          ...
          {!isGroup && (
            <div className="space-y-4 border-b pb-5">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm text-foreground font-medium">
                    {isFriend && displayEmail ? displayEmail : "Chỉ chia sẻ với bạn bè"}
                  </p>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm text-foreground font-medium">
                    {displayEmail ? `@${displayEmail.split("@")[0]}` : `@user_${friend.id}`}
                  </p>
          ...
          <div className="space-y-3.5 border-b pb-5">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 cursor-pointer hover:text-primary transition-colors">
                ...
                <span className="text-sm font-medium text-foreground">{isGroup ? "27 tệp ảnh" : "558 tệp ảnh"}</span>
              </div>
              <div className="flex items-center gap-3 cursor-pointer hover:text-primary transition-colors">
                <File className="size-5 shrink-0 text-muted-foreground/85" />
                <span className="text-sm font-medium text-foreground">{isGroup ? "3 tài liệu" : "12 tài liệu"}</span>
              </div>
              <div className="flex items-center gap-3 cursor-pointer hover:text-primary transition-colors">
                <LinkIcon className="size-5 shrink-0 text-muted-foreground/85" />
                <span className="text-sm font-medium text-foreground">{isGroup ? "26 liên kết" : "134 liên kết"}</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3.5">
              <div className="flex items-center justify-between">
                <h4 className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                  {t("chat.memberCount", { count: activeConversation.members.length })}
                </h4>
                <button
                  type="button"
                  onClick={() => handleActionClick("thêm thành viên")}
                  className="text-primary hover:text-primary/80 transition-colors p-0.5 active:scale-90 cursor-pointer"
              ...
              <div className="space-y-2.5 max-h-[300px] overflow-y-auto pr-1">
                {activeConversation.members.map((member) => {
                  const isMemberOnline = member.userId === currentUserId ? true : false
                  ...
                  return (
                    <div
                      ...
                      className="flex items-center gap-3 cursor-pointer hover:bg-muted/40 p-1.5 rounded-lg transition-colors"
                    >
                      <Avatar className="h-8.5 w-8.5 rounded-full shrink-0">
